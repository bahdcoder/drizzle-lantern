...
export const settings = mysqlTable("settings", {
  ...
  domain: varchar("domain", { length: 50 })
    .unique()
    .notNull(),
  installedSslCertificate: boolean(
    "installedSslCertificate"
  )
    .default(false)
    .notNull(),
});
...
export const accessTokens = mysqlTable("accessTokens", {
  id,
  userId: varchar("userId", { length: 32 }).references(
    () => users.id
  ),
  teamId: varchar("teamId", { length: 32 }).references(
    () => teams.id
  ),
...
export const teams = mysqlTable("teams", {
  ...
  configurationKey: varchar("configurationKey", {
    length: 512,
  }).notNull(),
  broadcastEditor: mysqlEnum("broadcastEditor", [
    "DEFAULT",
    "MARKDOWN",
  ]),
});
...
export const mailers = mysqlTable("mailers", {
  ...
  sendingEnabled: boolean("sendingEnabled")
    .default(false)
    .notNull(),
  inSandboxMode: boolean("inSandboxMode")
    .default(true)
    .notNull(),
...
export const mailerIdentities = mysqlTable(
  "mailerIdentities",
  {
    id,
    mailerId: varchar("mailerId", {
      length: 32,
...
export const audiences = mysqlTable("audiences", {
  ...
  teamId: varchar("teamId", { length: 32 })
    .references(() => teams.id)
    .notNull(),
  knownAttributesKeys: json("knownAttributes").$type<
    string[]
  >(),
});
...
export const contacts = mysqlTable(
  "contacts",
  {
    ...
    attributes:
      json("attributes").$type<Record<string, any>>(),
    createdAt: timestamp("createdAt").defaultNow(),
  },
  (table) => ({
    ContactEmailAudienceIdKey: unique(
      "ContactEmailAudienceIdKey"
...
export const sends = mysqlTable("sends", {
  ...
  contactId: varchar("contactId", { length: 32 })
    ...
    .notNull(),
  broadcastId: varchar("broadcastId", {
    length: 32,
...
export const broadcasts = mysqlTable("broadcasts", {
  ...
  winningAbTestVariantId: varchar(
...
export const automationSteps = mysqlTable(
  "automationSteps",
  {
    ...
    configuration: json("configuration")
      ...
      .notNull(),
    emailId: varchar("emailId", { length: 32 }).references(
      () => emails.id
    ),
    tagId: varchar("tagId", { length: 32 }).references(
      () => tags.id
    ),
...
export const emailRelations = relations(
  emails,
  ({ one }) => ({
    emailContent: one(emailContents, {
...
export const sendsRelations = relations(
  sends,
  ({ one, many }) => ({
...
export const mailerRelations = relations(
  mailers,
  ({ one, many }) => ({
    team: one(teams, {
      fields: [mailers.teamId],
      references: [teams.id],
    }),
...
export const WebhookRelations = relations(
  webhooks,
  ({ one }) => ({
    team: one(teams, {
...
export const AudienceRelations = relations(
  audiences,
  ({ one, many }) => ({
    team: one(teams, {
